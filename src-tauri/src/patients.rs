...
pub fn inactive_for_years(
    ...
    let mut st=conn.prepare(&format!("{PATIENT_SELECT} WHERE deleted_at IS NULL AND datetime(COALESCE(last_activity_at,updated_at,created_at)) < datetime('now',?1) ORDER BY datetime(COALESCE(last_activity_at,updated_at,created_at)) ASC LIMIT ?2")).map_err(|e|e.to_string())?;
    let rows = st
...
mod tests {
    ...
    fn db() -> Connection {
        ...
        .unwrap();
        c.execute_batch(include_str!("../migrations/005_patient_last_activity.sql"))
            .unwrap();
        c.execute_batch(include_str!(
    ...
    fn inactive_patients_are_detected_without_deletion() {
        ...
        c.execute("UPDATE patients SET created_at='2010-01-01 00:00:00',updated_at='2010-01-01 00:00:00',last_activity_at='2010-01-01 00:00:00' WHERE id=?1",[old.id]).unwrap();
    ...
    #[test]
    fn patient_update_refreshes_last_activity() {
        let mut c = db();
        let p = create(&mut c, input("مريض نشاط", "1234567890")).unwrap();
        c.execute(
            "UPDATE patients SET last_activity_at='2010-01-01 00:00:00' WHERE id=?1",
            [p.id],
        )
        .unwrap();
        update(&c, p.id, input("مريض نشاط محدث", "1234567890")).unwrap();
        let activity: String = c
            .query_row(
                "SELECT last_activity_at FROM patients WHERE id=?1",
                [p.id],
                |r| r.get(0),
            )
            .unwrap();

...
#[tauri::command]fn appointment_missed_history(db:tauri::State<Db>,to:String)->Result<Vec<appointments::Appointment>,String>{with_db(&db,|c|Ok(appointments::list(c,"1950-01-01T00:00:00",&to)?.into_iter().filter(|a|a.status=="no_show").collect()))}
#[tauri::command]fn appointment_upcoming_all(db:tauri::State<Db>,from:String)->Result<Vec<appointments::Appointment>,String>{with_db(&db,|c|appointments::list(c,&from,"2050-12-31T23:59:59"))}
...
#[cfg_attr(mobile,tauri::mobile_entry_point)]pub fn run(){tauri::Builder::default().plugin(tauri_plugin_dialog::init()).setup(|app|{let path=app.path().app_data_dir()?.join("clinicdesk.sqlite3");let db=init_db(&path).map_err(std::io::Error::other)?;app.manage(Db(Mutex::new(db)));Ok(())}).invoke_handler(tauri::generate_handler![health,patient_list,patient_count,patient_by_file_no,patient_inactive,patient_create,patient_update,patient_delete,patient_future_appointment_count,clinic_list,clinic_create,clinic_update,clinic_delete,doctor_list,doctor_create,doctor_update,doctor_delete,appointment_list,appointment_missed_history,appointment_upcoming_all,patient_appointments,appointment_create,appointment_update,appointment_status,visit_tracking_command::visit_tracking_get,visit_tracking_update,visit_follow_ups,attachment_list,attachment_import,attachment_remove,scheduling_settings_get,scheduling_settings_update,closure_list,closure_create,closure_delete]).run(tauri::generate_context!()).expect("error while running ClinicDesk");}
#[cfg(test)]mod migration_tests{use super::*;#[test]fn migrations_reach_latest_version_and_are_idempotent(){let db=Connection::open_in_memory().unwrap();migrate_db(&db).unwrap();assert_eq!(schema_version(&db).unwrap(),LATEST_SCHEMA_VERSION);migrate_db(&db).unwrap();assert_eq!(schema_version(&db).unwrap(),LATEST_SCHEMA_VERSION);}#[test]fn migration_rejects_future_schema_version(){let db=Connection::open_in_memory().unwrap();db.execute_batch(include_str!("../migrations/001_init.sql")).unwrap();db.execute("UPDATE app_meta SET value='99' WHERE key='schema_version'",[]).unwrap();assert!(migrate_db(&db).unwrap_err().contains("أحدث"));}#[test]fn migration_to_v10_restores_users_permissions_and_lifecycle(){let db=Connection::open_in_memory().unwrap();migrate_db(&db).unwrap();for table in["users","permissions","user_permissions","employee_lifecycle"]{let n:i64=db.query_row("SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name=?1",[table],|r|r.get(0)).unwrap();assert_eq!(n,1,"missing {table}");}let n:i64=db.query_row("SELECT COUNT(*) FROM permissions",[],|r|r.get(0)).unwrap();assert!(n>=16);}}

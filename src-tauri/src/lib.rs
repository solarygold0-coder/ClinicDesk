...
#[tauri::command]
fn patient_list(db: tauri::State<Db>, query: Option<String>, limit: Option<i64>) -> Result<Vec<patients::Patient>, String> { with_db(&db, |c| patients::list(c, query, limit.unwrap_or(50))) }
#[tauri::command]
fn patient_count(db: tauri::State<Db>) -> Result<i64, String> { with_db(&db, |c| c.query_row("SELECT COUNT(*) FROM patients WHERE deleted_at IS NULL", [], |r| r.get(0)).map_err(|e| e.to_string())) }
#[tauri::command]
fn patient_by_file_no(db: tauri::State<Db>, file_no: i64) -> Result<Option<patients::Patient>, String> { with_db(&db, |c| patients::get_by_file_no(c, file_no)) }
#[tauri::command]
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            health, patient_list, patient_count, patient_by_file_no, patient_inactive,
            patient_create, patient_update, patient_delete, patient_future_appointment_count,
            clinic_list, clinic_create, clinic_update, clinic_delete, doctor_list, doctor_create,
            doctor_update, doctor_delete, appointment_list, appointment_missed_history,
            appointment_upcoming_all, patient_appointments, appointment_create, appointment_update,
            appointment_status, visit_tracking_update, visit_tracking_command::visit_tracking_get,
            visit_follow_ups, attachment_list, attachment_import, attachment_archive,
            attachment_restore, scheduling_settings_get, scheduling_settings_update, closure_list,
            closure_create, closure_delete, backup_create, backup_restore, audit_recent
        ])

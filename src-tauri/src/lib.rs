...
#[tauri::command]
fn attachment_import(app:tauri::AppHandle,db:tauri::State<Db>,patient_id:i64,source_path:String)->Result<i64,String>{let root=attachment_root(&app)?;with_db(&db,|c|attachments::import_file(c,patient_id,Path::new(&source_path),&root))}
#[tauri::command]
fn attachment_remove(app:tauri::AppHandle,db:tauri::State<Db>,id:i64)->Result<(),String>{let root=attachment_root(&app)?;with_db(&db,|c|attachments::remove_file(c,id,&root))}
#[tauri::command]
...
fn backup_restore(app:tauri::AppHandle,db:tauri::State<Db>,source_path:String)->Result<String,String>{
    ...
    let mut guard=db.0.lock().map_err(|_|"تعذر الوصول إلى قاعدة البيانات".to_string())?;
    audit::record(&guard,"restore_started","database",None,Some(&serde_json::json!({"sha256":source_hash}).to_string()))?;
    let hash=backup::restore_database(&mut guard,source,&live,LATEST_SCHEMA_VERSION)?;
    audit::record(&guard,"restore_completed","database",None,Some(&serde_json::json!({"sha256":hash}).to_string()))?;
    Ok(hash)
}

pub fn run(){tauri::Builder::default().plugin(tauri_plugin_dialog::init()).plugin(tauri_plugin_opener::init()).setup(|app|{let db_path=app.path().app_data_dir().map_err(|e|e.to_string())?.join("clinicdesk.sqlite3");let db=init_db(&db_path)?;app.manage(Db(Mutex::new(db)));Ok(())}).invoke_handler(tauri::generate_handler![health,patient_list,patient_count,patient_by_file_no,patient_inactive,patient_create,patient_update,patient_delete,patient_future_appointment_count,clinic_list,clinic_create,clinic_update,clinic_delete,doctor_list,doctor_create,doctor_update,doctor_delete,appointment_list,appointment_missed_history,appointment_upcoming_all,patient_appointments,appointment_create,appointment_update,appointment_status,visit_tracking_update,visit_follow_ups,attachment_list,attachment_import,attachment_remove,scheduling_settings_get,scheduling_settings_update,closure_list,closure_create,closure_delete,backup_create,backup_restore,visit_tracking_command::visit_tracking_update_cmd]).run(tauri::generate_context!()).expect("error while running ClinicDesk");}

...
fn migrate_db(db: &Connection) -> Result<(), String> {
    // Migration 001 is the bootstrap migration. It is deliberately idempotent
    // so it can create app_meta before we inspect the current schema version.
...
#[tauri::command]
fn security_state(db: tauri::State<Db>) -> Result<security::SecurityState, String> { with_db(&db, security::state) }
#[tauri::command]
...
mod migration_tests {
    ...
    fn newer_database_is_rejected() {
        ...
        db.execute("UPDATE app_meta SET value='99' WHERE key='schema_version'", []).unwrap();

...
fn backup_restore(
    ...
    let started = serde_json::json!({"sha256":source_hash}).to_string();
    security_log::append(
        &security_log_path(&app)?,
        "restore",
        "started",
    ...
    let mut g =
        db.0.lock()
            .map_err(|_| "تعذر الوصول إلى قاعدة البيانات".to_string())?;
    let details = serde_json::json!({"sha256":source_hash,"restoreRef":restore_ref}).to_string();
    audit::record(&g, "restore_started", "database", None, Some(&details))?;
    let live_path = app
        .path()
        .app_data_dir()
        .map_err(|e| e.to_string())?

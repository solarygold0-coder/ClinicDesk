...
#[tauri::command] fn backup_restore(app:tauri::AppHandle,db:tauri::State<Db>,source_path:String)->Result<String,String>{let live=app.path().app_data_dir().map_err(|e|e.to_string())?.join("clinicdesk.sqlite3");let source=Path::new(&source_path);backup::verify_database(source,LATEST_SCHEMA_VERSION)?;backup::verify_attachment_backup(source)?;let source_hash=backup::sha256_file(source)?;let mut guard=db.0.lock().map_err(|_|"تعذر الوصول إلى قاعدة البيانات".to_string())?;audit::record(&guard,"restore_started","database",None,Some(&serde_json::json!({"sha256":source_hash}).to_string()))?;let hash=backup::restore_database(&mut guard,source,&live,LATEST_SCHEMA_VERSION)?;audit::record(&guard,"restore_completed","database",None,Some(&serde_json::json!({"sha256":hash}).to_string()))?;Ok(hash)}
...
pub fn run(){
 tauri::Builder::default().plugin(tauri_plugin_dialog::init()).plugin(tauri_plugin_opener::init()).setup(|app|{
  let db_path=app.path().app_data_dir().map_err(|e|e.to_string())?.join("clinicdesk.sqlite3");
  let db=init_db(&db_path)?;app.manage(Db(Mutex::new(db)));Ok(())
 ...
 attachment_list,attachment_import,attachment_remove,scheduling_settings_get,scheduling_settings_update,closure_list,closure_create,closure_delete,

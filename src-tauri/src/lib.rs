...
fn init_db(path: &PathBuf) -> Result<Connection, String> {
    ...
    let schema_version: i64 = db
        ...
        .map_err(|e| e.to_string())?;
    if schema_version < 5 {
        db.execute_batch(include_str!("../migrations/005_patient_last_activity.sql"))
            .map_err(|e| e.to_string())?;
    }
    let schema_version: i64 = db
        .query_row(
    ...
    if schema_version < 6 {
        db.execute_batch(include_str!(
            "../migrations/006_patient_activity_triggers.sql"
        ))
        .map_err(|e| e.to_string())?;
    }
    let schema_version: i64 = db
...
#[tauri::command]
fn appointment_missed_history(db:tauri::State<Db>,to:String)->Result<Vec<appointments::Appointment>,String>{with_db(&db,|c|{let rows=appointments::list(c,"1900-01-01T00:00:00",&to)?;Ok(rows.into_iter().filter(|a|a.status=="no_show").collect())})}
#[tauri::command]

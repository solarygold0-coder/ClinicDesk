...
#[tauri::command]
fn backup_restore(app: tauri::AppHandle, db: tauri::State<Db>, source_path: String) -> Result<String, String> { let live = app.path().app_data_dir().map_err(|e| e.to_string())?.join("clinicdesk.sqlite3"); let g = db.0.lock().map_err(|_| "تعذر الوصول إلى قاعدة البيانات".to_string())?; let hash = backup::validate_database_backup(Path::new(&source_path), LATEST_SCHEMA_VERSION)?; audit::record(&g, "restore_started", "database", None, Some(&serde_json::json!({"sha256":hash}).to_string()))?; let safety = live.with_extension("pre-restore.sqlite3"); backup::create_database_backup(&g, &safety, LATEST_SCHEMA_VERSION)?; drop(g); fs::copy(&source_path, &live).map_err(|e| e.to_string())?; let reopened = init_db(&live)?; let mut guard = db.0.lock().map_err(|_| "تعذر الوصول إلى قاعدة البيانات".to_string())?; *guard = reopened; audit::record(&guard, "restore_completed", "database", None, Some(&serde_json::json!({"sha256":hash}).to_string()))?; Ok(hash) }

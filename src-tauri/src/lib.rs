...
fn backup_create(app: tauri::AppHandle, db: tauri::State<Db>, destination_path: String) -> Result<String, String> {
    ...
    security_log::append(&security_log_path(&app)?, "backup", "success", None, Some(&details))?;
    Ok(hash)
}
...
fn backup_restore(app: tauri::AppHandle, db: tauri::State<Db>, source_path: String) -> Result<String, String> {
    ...
    security_log::append(&security_path, "restore", "started", Some(&restore_ref), Some(&start_details))?;
    ...
    let restore_result = (|| -> Result<String, String> {
        ...
        audit::record(&guard, "restore_started", "database", None, Some(&serde_json::json!({"sha256": source_hash, "restore_ref": restore_ref}).to_string()))?;
        let hash = backup::restore_database(&mut guard, source, &live, LATEST_SCHEMA_VERSION)?;
        audit::record(&guard, "restore_completed", "database", None, Some(&serde_json::json!({"sha256": hash, "restore_ref": restore_ref}).to_string()))?;
        Ok(hash)
    })();
    ...
    match restore_result {
        Ok(hash) => {
            let details = serde_json::json!({"sha256": hash}).to_string();
            security_log::append(&security_path, "restore", "success", Some(&restore_ref), Some(&details))?;
            Ok(hash)
        }
        Err(error) => {
            let details = serde_json::json!({"error": error}).to_string();
            security_log::append(&security_path, "restore", "failure", Some(&restore_ref), Some(&details))?;
            Err(error)
        }
    }
}

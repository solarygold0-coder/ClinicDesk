...
fn migrate_db(db: &Connection) -> Result<(), String> {
    ...
    for (version, sql) in [
        ...
        if schema_version(db)? < version {
            ...
            if applied != version {
                return Err(format!("فشل ترحيل قاعدة البيانات إلى الإصدار {version}: الإصدار المسجل {applied}"));
            }
        }
    }
    let final_version = schema_version(db)?;
    if final_version != LATEST_SCHEMA_VERSION {
        return Err(format!("إصدار قاعدة البيانات بعد الترحيل {final_version} بدلاً من {LATEST_SCHEMA_VERSION}"));
    }
    Ok(())
}

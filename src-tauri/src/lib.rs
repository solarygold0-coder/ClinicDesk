...
fn init_db(path:&PathBuf)->Result<Connection,String>{if let Some(p)=path.parent(){fs::create_dir_all(p).map_err(|e|e.to_string())?}let db=Connection::open(path).map_err(|e|e.to_string())?;db.execute_batch(include_str!("../migrations/001_init.sql")).map_err(|e|e.to_string())?;db.execute_batch(include_str!("../migrations/002_touch_triggers.sql")).map_err(|e|e.to_string())?;db.execute_batch(include_str!("../migrations/003_scheduling_rules.sql")).map_err(|e|e.to_string())?;for(v,sql)in[(4,include_str!("../migrations/004_patient_medical_details.sql")),(5,include_str!("../migrations/005_patient_last_activity.sql")),(6,include_str!("../migrations/006_patient_activity_triggers.sql")),(7,include_str!("../migrations/007_appointment_visit_tracking.sql")),(8,include_str!("../migrations/008_optional_auth.sql"))]{let current:i64=db.query_row("SELECT CAST(value AS INTEGER) FROM app_meta WHERE key='schema_version'",[],|r|r.get(0)).map_err(|e|e.to_string())?;if current<v{db.execute_batch(sql).map_err(|e|e.to_string())?}}Ok(db)}

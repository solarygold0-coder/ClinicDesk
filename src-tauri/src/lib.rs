...
#[tauri::command]
fn appointment_status(db:tauri::State<Db>,id:i64,status:String)->Result<(),String>{with_db(&db,|c|appointments::set_status(c,id,&status))}
#[tauri::command]
...
pub fn run(){
 tauri::Builder::default().setup(|app|{let path=app.path().app_data_dir()?.join("clinicdesk.sqlite3");let db=init_db(&path).map_err(std::io::Error::other)?;app.manage(Db(Mutex::new(db)));Ok(())})
 .invoke_handler(tauri::generate_handler![health,security_state,patient_list,patient_count,patient_by_file_no,patient_inactive,patient_create,patient_update,patient_delete,patient_future_appointment_count,clinic_list,clinic_create,clinic_update,clinic_delete,doctor_list,doctor_create,doctor_update,doctor_delete,appointment_list,appointment_missed_history,appointment_upcoming_all,patient_appointments,appointment_create,appointment_update,appointment_status,visit_tracking_update,visit_follow_ups,attachment_list,attachment_import,attachment_remove,scheduling_settings_get,scheduling_settings_update,closure_list,closure_create,closure_delete])

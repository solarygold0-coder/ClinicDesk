...
fn init_db(path: &PathBuf) -> Result<Connection, String> {
    ...
    if schema_version < 4 {
        db.execute_batch(include_str!("../migrations/004_patient_medical_details.sql")).map_err(|e| e.to_string())?;
    }
    Ok(db)
}
...
#[tauri::command]
fn patient_list(db: tauri::State<Db>, query: Option<String>, limit: Option<i64>) -> Result<Vec<patients::Patient>, String> {
    with_db(&db, |c| patients::list(c, query, limit.unwrap_or(50)))
}
#[tauri::command]
fn patient_count(db: tauri::State<Db>) -> Result<i64, String> {
    with_db(&db, |c| c.query_row("SELECT COUNT(*) FROM patients WHERE deleted_at IS NULL", [], |r| r.get(0)).map_err(|e| e.to_string()))
}
#[tauri::command]
fn patient_by_file_no(db: tauri::State<Db>, file_no: i64) -> Result<Option<patients::Patient>, String> {
    with_db(&db, |c| patients::get_by_file_no(c, file_no))
}
#[tauri::command]
fn patient_inactive(db: tauri::State<Db>, years: Option<i64>, limit: Option<i64>) -> Result<Vec<patients::Patient>, String> {
    with_db(&db, |c| patients::inactive_for_years(c, years.unwrap_or(10), limit.unwrap_or(100)))
}
#[tauri::command]
fn patient_create(db: tauri::State<Db>, input: patients::PatientInput) -> Result<patients::Patient, String> {
    let mut g = db.0.lock().map_err(|_| "تعذر الوصول إلى قاعدة البيانات".to_string())?;
    patients::create(&mut g, input)
}
#[tauri::command]
fn patient_update(db: tauri::State<Db>, id: i64, input: patients::PatientInput) -> Result<patients::Patient, String> {
...
#[tauri::command]
fn appointment_missed_history(db: tauri::State<Db>, to: String) -> Result<Vec<appointments::Appointment>, String> {
    with_db(&db, |c| {
...
#[tauri::command]
fn patient_appointments(db: tauri::State<Db>, patient_id: i64, limit: Option<i64>) -> Result<Vec<appointments::Appointment>, String> {
    with_db(&db, |c| appointments::list_for_patient(c, patient_id, limit.unwrap_or(30)))
}
#[tauri::command]
fn appointment_create(db: tauri::State<Db>, input: appointments::AppointmentInput) -> Result<appointments::Appointment, String> {
    let mut g = db.0.lock().map_err(|_| "تعذر الوصول إلى قاعدة البيانات".to_string())?;
    appointments::create(&mut g, input)
}
#[tauri::command]
fn appointment_update(db: tauri::State<Db>, id: i64, input: appointments::AppointmentInput) -> Result<appointments::Appointment, String> {
    let mut g = db.0.lock().map_err(|_| "تعذر الوصول إلى قاعدة البيانات".to_string())?;

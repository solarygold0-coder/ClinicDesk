...
fn init_db(path: &PathBuf) -> Result<Connection, String> {
    ...
    let schema_version: i64 = db
        ...
        .map_err(|e| e.to_string())?;
    if schema_version < 6 {
        db.execute_batch(include_str!("../migrations/006_patient_activity_triggers.sql"))
            .map_err(|e| e.to_string())?;
    }
    Ok(db)
}

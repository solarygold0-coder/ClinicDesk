...
#[tauri::command] fn backup_create(db:tauri::State<Db>,destination_path:String)->Result<String,String>{let g=db.0.lock().map_err(|_|"تعذر الوصول إلى قاعدة البيانات".to_string())?;let hash=backup::create_database_backup(&g,Path::new(&destination_path),LATEST_SCHEMA_VERSION)?;let details=serde_json::json!({"sha256":hash}).to_string();audit::record(&g,"backup_created","database",None,Some(&details))?;Ok(hash)}
#[tauri::command] fn backup_restore(app:tauri::AppHandle,db:tauri::State<Db>,source_path:String)->Result<String,String>{let live=app.path().app_data_dir().map_err(|e|e.to_string())?.join("clinicdesk.sqlite3");let mut g=db.0.lock().map_err(|_|"تعذر الوصول إلى قاعدة البيانات".to_string())?;let hash=backup::restore_database(&mut g,Path::new(&source_path),&live,LATEST_SCHEMA_VERSION)?;let details=serde_json::json!({"sha256":hash}).to_string();audit::record(&g,"backup_restored","database",None,Some(&details))?;Ok(hash)}
...
mod migration_tests {
 use super::*;
 fn table_exists(db:&Connection,name:&str)->i64{db.query_row("SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name=?1",[name],|r|r.get(0)).unwrap()}
 #[test] fn migrations_reach_latest_version_and_are_idempotent(){let db=Connection::open_in_memory().unwrap();migrate_db(&db).unwrap();assert_eq!(schema_version(&db).unwrap(),10);migrate_db(&db).unwrap();assert_eq!(schema_version(&db).unwrap(),10);assert_eq!(table_exists(&db,"users"),1);assert_eq!(table_exists(&db,"roles"),1);}
 #[test] fn migration_rejects_future_schema_version(){let db=Connection::open_in_memory().unwrap();db.execute_batch(include_str!("../migrations/001_init.sql")).unwrap();db.execute("UPDATE app_meta SET value='99' WHERE key='schema_version'",[]).unwrap();assert!(migrate_db(&db).unwrap_err().contains("أحدث"));}
 #[test] fn migration_from_v9_restores_auth_and_preserves_data(){
  let db=Connection::open_in_memory().unwrap();
  db.execute_batch(include_str!("../migrations/001_init.sql")).unwrap();
  for sql in [include_str!("../migrations/002_touch_triggers.sql"),include_str!("../migrations/003_scheduling_rules.sql"),include_str!("../migrations/004_patient_medical_details.sql"),include_str!("../migrations/005_patient_last_activity.sql"),include_str!("../migrations/006_patient_activity_triggers.sql"),include_str!("../migrations/007_appointment_visit_tracking.sql"),include_str!("../migrations/008_optional_auth.sql"),include_str!("../migrations/009_remove_optional_auth.sql")] {db.execute_batch(sql).unwrap();}
  db.execute("INSERT INTO patients(file_no,full_name,national_id) VALUES(9,'مريض محفوظ من إصدار 9','9234567890')",[]).unwrap();
  assert_eq!(schema_version(&db).unwrap(),9);assert_eq!(table_exists(&db,"users"),0);
  migrate_db(&db).unwrap();
  assert_eq!(schema_version(&db).unwrap(),10);assert_eq!(table_exists(&db,"users"),1);assert_eq!(table_exists(&db,"roles"),1);
  let name:String=db.query_row("SELECT full_name FROM patients WHERE file_no=9",[],|r|r.get(0)).unwrap();assert_eq!(name,"مريض محفوظ من إصدار 9");
  let actor_cols:i64=db.query_row("SELECT COUNT(*) FROM pragma_table_info('audit_log') WHERE name IN ('actor_user_id','actor_display_name')",[],|r|r.get(0)).unwrap();assert_eq!(actor_cols,2);
 }
}

...
pub fn list(conn: &Connection, patient_id: i64) -> Result<Vec<Attachment>, String> {
    ...
    let rows = stmt
        .query_map(params![patient_id], |r| {
            Ok(Attachment {
                id: r.get(0)?, patient_id: r.get(1)?, stored_name: r.get(2)?, original_name: r.get(3)?,
                mime_type: r.get(4)?, size_bytes: r.get(5)?, sha256: r.get(6)?, created_at: r.get(7)?,
            })
...
fn safe_stored_path(root: &Path, stored_name: &str) -> Result<PathBuf, String> {
    if stored_name.is_empty() || Path::new(stored_name).file_name().and_then(|v| v.to_str()) != Some(stored_name) {
...
pub fn remove_file(conn: &Connection, id: i64, root: &Path) -> Result<(), String> {
    ...
    if moved { fs::rename(&path, &quarantine).map_err(|e| e.to_string())?; }
    if let Err(e) = remove(conn, id) {
        if moved {
            fs::rename(&quarantine, &path).map_err(|restore| format!("{e}; تعذر استعادة المرفق بعد فشل الحذف: {restore}"))?;
        }
        return Err(e);
    }
    if moved { fs::remove_file(&quarantine).map_err(|e| format!("تم حذف سجل المرفق لكن تعذر تنظيف الملف المؤقت: {e}"))?; }
    Ok(())
}
...
mod tests {
    ...
    fn attachment_metadata_round_trip() {
        let conn = db();
        let id = add(&conn, 1, "abc.pdf", "report.pdf", Some("application/pdf"), 1024, &"a".repeat(64)).unwrap();
    ...
    fn rejects_oversized_attachment() {
        let conn = db();
        assert!(add(&conn, 1, "x.pdf", "x.pdf", None, 26 * 1024 * 1024, &"a".repeat(64)).is_err());
    }
    #[test]

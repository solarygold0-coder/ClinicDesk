...
pub fn list(conn: &Connection, patient_id: i64) -> Result<Vec<Attachment>, String> {
    let mut stmt = conn
        .prepare("SELECT id,patient_id,stored_name,original_name,COALESCE(display_name,original_name),category,mime_type,size_bytes,sha256,created_at,deleted_at,deleted_reason FROM attachments WHERE patient_id=?1 AND deleted_at IS NULL ORDER BY created_at DESC,id DESC")
...
mod tests {
    ...
    fn blocks_executable_extensions() {
        assert!(extension(Path::new("evil.exe")).is_err());
        assert!(extension(Path::new("script.ps1")).is_err());
        assert!(extension(Path::new("report.pdf")).is_ok());
    }

...
fn validate(input: &VisitTrackingInput) -> Result<(), String> {
    ...
    if input.follow_up_at.as_deref().is_some_and(|value| value.trim().is_empty()) {
        return Err("تاريخ المتابعة غير صالح".into());
    }
    Ok(())
}

pub fn get(c: &Connection, id: i64) -> Result<VisitTrackingInput, String> {
    c.query_row(
        "SELECT visit_type,visit_stage,follow_up_at FROM appointments WHERE id=?1",
        params![id],
        |row| Ok(VisitTrackingInput { visit_type: row.get(0)?, visit_stage: row.get(1)?, follow_up_at: row.get(2)? }),
    )
    .map_err(|e| if matches!(e, rusqlite::Error::QueryReturnedNoRows) { "الموعد غير موجود".into() } else { e.to_string() })
}

pub fn update(c: &Connection, id: i64, input: VisitTrackingInput) -> Result<(), String> {
    validate(&input)?;
    let follow_up_at = input.follow_up_at.map(|value| value.trim().to_string()).filter(|value| !value.is_empty());
    ...
    if changed == 0 { return Err("الموعد غير موجود".into()); }
    c.execute(
...
pub fn follow_ups(c: &Connection, from: &str, to: &str) -> Result<Vec<FollowUpVisit>, String> {
    ...
    let rows = statement.query_map(params![from, to], |row| Ok(FollowUpVisit {
        appointment_id: row.get(0)?, patient_id: row.get(1)?, file_no: row.get(2)?, patient_name: row.get(3)?, follow_up_at: row.get(4)?, visit_type: row.get(5)?,
    })).map_err(|e| e.to_string())?;
...
mod tests {
    ...
    fn db() -> Connection {
        ...
        c.execute_batch(include_str!("../migrations/007_appointment_visit_tracking.sql")).unwrap();
        c.execute("INSERT INTO patients(file_no,full_name) VALUES(1,'مريض')", []).unwrap();
        c.execute("INSERT INTO appointments(patient_id,starts_at,ends_at) VALUES(1,'2026-09-20T10:00:00','2026-09-20T10:30:00')", []).unwrap();
        c
    }
    #[test]
    fn visit_tracking_is_saved_and_follow_up_is_queryable() {
        let c = db();
        update(&c, 1, VisitTrackingInput { visit_type: "follow_up".into(), visit_stage: "reception".into(), follow_up_at: Some("2026-10-01T10:00:00".into()) }).unwrap();
        let current = get(&c, 1).unwrap();
        assert_eq!(current.visit_type, "follow_up");
        assert_eq!(current.visit_stage, "reception");
        assert_eq!(current.follow_up_at.as_deref(), Some("2026-10-01T10:00:00"));
    ...
    fn invalid_visit_type_is_rejected() {
        let c = db();
        let result = update(&c, 1, VisitTrackingInput { visit_type: "invalid".into(), visit_stage: "scheduled".into(), follow_up_at: None });

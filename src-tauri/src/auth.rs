...
const VALID_ROLES: &[&str] = &["ordinary_employee", "general_manager", "deputy_manager", "doctor", "specialist"];
const VALID_STATUSES: &[&str] = &["ACTIVE", "SUSPENDED", "CLOSED_INACTIVITY", "ENDED_SERVICE", "RETIRED"];
...
fn required(value: &str, field: &str) -> Result<String, String> {
    ...
    if value.is_empty() { return Err(format!("{field} مطلوب")); }
    Ok(value.to_string())
}

fn valid_role(role: &str) -> Result<&str, String> {
    if VALID_ROLES.contains(&role) { Ok(role) } else { Err("نوع المستخدم غير صالح".into()) }
}

fn valid_status(status: &str) -> Result<&str, String> {
    if VALID_STATUSES.contains(&status) { Ok(status) } else { Err("حالة المستخدم غير صالحة".into()) }
}
...
pub fn hash_password(password: &str) -> Result<String, String> {
    ...
    let salt = SaltString::encode_b64(Uuid::new_v4().as_bytes()).map_err(|_| "تعذر إنشاء بصمة كلمة المرور".to_string())?;
    Argon2::default().hash_password(password.as_bytes(), &salt).map(|h| h.to_string()).map_err(|_| "تعذر إنشاء بصمة كلمة المرور".to_string())
}

fn verify_password(password: &str, encoded: &str) -> Result<bool, String> {
    let parsed = PasswordHash::new(encoded).map_err(|_| "صيغة كلمة المرور قديمة وتحتاج إعادة تعيين".to_string())?;
...
pub fn list_users(db: &Connection) -> Result<Vec<UserSummary>, String> {
    ...
    let rows = stmt.query_map([], |r| Ok(UserSummary { id:r.get(0)?, username:r.get(1)?, display_name:r.get(2)?, employee_code:r.get(3)?, role_type:r.get(4)?, account_status:r.get(5)?, must_change_password:r.get::<_,i64>(6)? != 0 })).map_err(|e| e.to_string())?;
    rows.collect::<Result<Vec<_>,_>>().map_err(|e| e.to_string())
}

pub fn create_user(db: &mut Connection, username: String, display_name: String, password: String, role_type: String) -> Result<UserSummary, String> {
    let role = valid_role(role_type.trim())?.to_string();
    let password_hash = hash_password(&password)?;
    let created = users::create(db, users::NewUserAccount { username, display_name, password_hash, is_system_admin: Some(is_admin_role(&role)) })?;
    db.execute("UPDATE users SET role_type=?2,is_system_admin=?3 WHERE id=?1", params![created.id, role, is_admin_role(&role) as i64]).map_err(|e| {
        let text=e.to_string();
        if text.contains("role_limit_") { "تم بلوغ الحد الأقصى لهذا الدور الوظيفي".to_string() } else { text }
    })?;
    summary_by_id(db, created.id)
}

pub fn update_user(db: &Connection, id: i64, username: String, display_name: String, role_type: String) -> Result<UserSummary, String> {
    ...
    if changed == 0 { return Err("المستخدم غير موجود".into()); }
    summary_by_id(db, id)
}

pub fn set_status(db: &Connection, id: i64, status: String, reason: Option<String>) -> Result<(), String> {
    ...
    if changed == 0 { return Err("المستخدم غير موجود".into()); }
    if !active { revoke_sessions(db, id)?; }
    Ok(())
}
...
pub fn authenticate(db: &Connection, username: String, password: String) -> Result<AuthSession, String> {
    let username = required(&username, "اسم المستخدم")?;
    ...
    if is_active == 0 || status != "ACTIVE" { return Err("الحساب غير نشط".into()); }
    ...
    if inactive != 0 {
        set_status(db, id, "CLOSED_INACTIVITY".into(), Some("90_day_inactivity".into()))?;
        return Err("الحساب مغلق بسبب عدم النشاط ويحتاج إعادة تفعيل".into());
    }
    if !verify_password(&password, &hash)? { return Err("بيانات الدخول غير صحيحة".into()); }
    ...
    db.execute("INSERT INTO auth_sessions(id,user_id,expires_at) VALUES(?1,?2,?3)", params![token,id,expires_at]).map_err(|e| e.to_string())?;
    Ok(AuthSession { token, expires_at, user: summary_by_id(db,id)? })
}
...
pub fn logout(db: &Connection, token: String) -> Result<(), String> {
    let token = required(&token, "رمز الجلسة")?;
    db.execute("UPDATE auth_sessions SET revoked_at=COALESCE(revoked_at,CURRENT_TIMESTAMP) WHERE id=?1", [token]).map_err(|e| e.to_string())?;
    Ok(())
}
...
mod tests {
    ...
    #[test] fn hashes_and_authenticates_without_plaintext(){ let mut db=db(); let u=create_user(&mut db,"gm".into(),"مدير".into(),"StrongPassword1".into(),"general_manager".into()).unwrap(); let stored:String=db.query_row("SELECT password_hash FROM users WHERE id=?1",[u.id],|r|r.get(0)).unwrap(); assert!(stored.starts_with("$argon2")); assert!(!stored.contains("StrongPassword1")); let s=authenticate(&db,"GM".into(),"StrongPassword1".into()).unwrap(); assert_eq!(validate_session(&db,s.token).unwrap().employee_code,"U01"); }
    #[test] fn wrong_password_is_rejected(){ let mut db=db(); create_user(&mut db,"user".into(),"موظف".into(),"StrongPassword1".into(),"ordinary_employee".into()).unwrap(); assert!(authenticate(&db,"user".into(),"wrong".into()).is_err()); }
    #[test] fn reset_revokes_sessions_and_forces_change(){ let mut db=db(); let u=create_user(&mut db,"user".into(),"موظف".into(),"StrongPassword1".into(),"ordinary_employee".into()).unwrap(); let s=authenticate(&db,"user".into(),"StrongPassword1".into()).unwrap(); reset_password(&db,u.id,"TemporaryPassword2".into()).unwrap(); assert!(validate_session(&db,s.token).is_err()); let s2=authenticate(&db,"user".into(),"TemporaryPassword2".into()).unwrap(); assert!(s2.user.must_change_password); }
    #[test] fn non_active_status_revokes_session(){ let mut db=db(); let u=create_user(&mut db,"user".into(),"موظف".into(),"StrongPassword1".into(),"ordinary_employee".into()).unwrap(); let s=authenticate(&db,"user".into(),"StrongPassword1".into()).unwrap(); set_status(&db,u.id,"RETIRED".into(),Some("service_end".into())).unwrap(); assert!(validate_session(&db,s.token).is_err()); }
}

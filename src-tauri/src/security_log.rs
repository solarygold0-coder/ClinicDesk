...
/// Converts an operational error into a stable, non-sensitive security code.
/// The original error remains available to the caller, but must never be copied
/// verbatim into the external tamper-evident log.
...
mod tests {
    ...
    fn failure_codes_never_echo_operational_errors() {
        ...
        assert_eq!(failure_code("foreign key check failed"), "foreign_key_failed");
        assert_eq!(failure_code("attachment hash mismatch"), "hash_failed");
        assert_eq!(failure_code("unexpected internal condition"), "operation_failed");
    }

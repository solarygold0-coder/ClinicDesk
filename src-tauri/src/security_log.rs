...
use std::{fs, io::Write, path::Path, sync::{Mutex, OnceLock}};
use uuid::Uuid;
...
fn safe_detail(details: Option<&str>) -> Option<String> {
    details.map(|raw| {
        ...
        for marker in ["password", "passwd", "token", "secret", "authorization", "bearer"] {
            if value.to_ascii_lowercase().contains(marker) { return "تفاصيل حساسة محجوبة".to_string(); }
        }
        ...
        if value.chars().count() > MAX_DETAIL_CHARS {
            value = value.chars().take(MAX_DETAIL_CHARS).collect(); value.push('…');
        }
        value
    })
}
...
pub fn verify(path: &Path) -> Result<String, String> {
    if !path.exists() { return Ok("GENESIS".to_string()); }
    let content = fs::read_to_string(path).map_err(|e| e.to_string())?;
    let mut expected_previous = "GENESIS".to_string();
    for line in content.lines().filter(|line| !line.trim().is_empty()) {
        let value: serde_json::Value = serde_json::from_str(line).map_err(|_| "سجل الأمان الخارجي تالف ولا يمكن التحقق من سلسلة البصمات".to_string())?;
        let previous = value.get("previous_hash").and_then(|v| v.as_str()).ok_or_else(|| "سجل الأمان الخارجي لا يحتوي بصمة سابقة صالحة".to_string())?;
        if previous != expected_previous { return Err("تم اكتشاف انقطاع أو تعديل في سلسلة سجل الأمان الخارجي".to_string()); }
        let stored = value.get("event_hash").and_then(|v| v.as_str()).ok_or_else(|| "سجل الأمان الخارجي لا يحتوي بصمة حدث صالحة".to_string())?;
        if stored != canonical_hash(&value)? { return Err("تم اكتشاف تعديل في محتوى سجل الأمان الخارجي".to_string()); }
...
pub fn append(path: &Path, event_type: &str, status: &str, reference: Option<&str>, details: Option<&str>) -> Result<String, String> {
    ...
    let _guard = APPEND_LOCK.get_or_init(|| Mutex::new(())).lock().map_err(|_| "تعذر قفل سجل الأمان الخارجي".to_string())?;
    ...
    let event = SecurityEvent { event_id: canonical["event_id"].as_str().unwrap().to_string(), occurred_at: canonical["occurred_at"].as_str().unwrap().to_string(), event_type, status, reference, details: safe_details.as_deref(), previous_hash: canonical["previous_hash"].as_str().unwrap().to_string(), event_hash };
...
mod tests {
    ...
    fn events_form_a_hash_chain_and_detect_content_tampering() {
        ...
        assert_ne!(original,tampered); fs::write(&path,tampered).unwrap();
        assert!(verify(&path).is_err()); assert!(append(&path,"backup","success",None,None).is_err());
    ...
    fn detects_deleted_or_reordered_chain_entries() {
        let path=std::env::temp_dir().join(format!("clinicdesk-security-{}.jsonl",Uuid::new_v4()));
        append(&path,"restore","started",Some("RST-2"),None).unwrap(); append(&path,"restore","success",Some("RST-2"),None).unwrap();
        let content=fs::read_to_string(&path).unwrap(); let lines:Vec<&str>=content.lines().collect(); fs::write(&path,format!("{}\n",lines[1])).unwrap();
        assert!(verify(&path).is_err()); let _=fs::remove_file(path);
    }
    ...
    fn concurrent_appends_preserve_one_linear_chain() {
        ...
        for i in 0..12 {
            ...
            workers.push(std::thread::spawn(move || append(path.as_path(),"backup","started",Some(&format!("BKP-{i}")),None).unwrap()));
        }

...
pub fn append(
    ...
    let _guard = APPEND_LOCK.get_or_init(|| Mutex::new(())).lock()
...
mod tests {
    ...
    fn strict_operation_contract_requires_reference_and_structured_details() {
        let path = std::env::temp_dir().join(format!("clinicdesk-security-contract-{}.jsonl", Uuid::new_v4()));
        assert!(append_operation(&path, "backup", "started", "", "preflight", "started", None).is_err());
        assert!(append_operation(&path, "restore", "started", "BKP-wrong", "preflight", "started", None).is_err());
        append_operation(&path, "backup", "started", "BKP-123", "preflight", "started", None).unwrap();
        append_operation(&path, "backup", "success", "BKP-123", "completed", "ok", Some("abc")).unwrap();
        verify(&path).unwrap();
        let content = fs::read_to_string(&path).unwrap();

...
pub fn append_operation(
    ...
    if !valid_operation_event(event_type, status, reference)
        || !valid_token(stage)
        || !valid_token(code)
        || sha256.is_some_and(|h| !valid_sha256(h))
    {
...
fn canonical_hash(value: &serde_json::Value) -> Result<String, String> {
    let canonical = serde_json::json!({"event_id":value.get("event_id").and_then(|v|v.as_str()).ok_or_else(||"سجل الأمان الخارجي يفتقد معرف الحدث".to_string())?,"occurred_at":value.get("occurred_at").and_then(|v|v.as_str()).ok_or_else(||"سجل الأمان الخارجي يفتقد وقت الحدث".to_string())?,"event_type":value.get("event_type").and_then(|v|v.as_str()).ok_or_else(||"سجل الأمان الخارجي يفتقد نوع الحدث".to_string())?,"status":value.get("status").and_then(|v|v.as_str()).ok_or_else(||"سجل الأمان الخارجي يفتقد حالة الحدث".to_string())?,"reference":value.get("reference").cloned().unwrap_or(serde_json::Value::Null),"details":value.get("details").cloned().unwrap_or(serde_json::Value::Null),"previous_hash":value.get("previous_hash").and_then(|v|v.as_str()).ok_or_else(||"سجل الأمان الخارجي يفتقد بصمة الحدث السابق".to_string())?});
    Ok(hash_bytes(
...
mod tests {
    ...
    fn failure_codes_never_echo_operational_errors() {
        assert_eq!(
            failure_code("failed to open C:/Users/Alice/Private/patient.db"),
            "io_failed"
        );
        assert_eq!(
    ...
    #[test]
    fn strict_operation_contract_rejects_unstructured_tokens_and_bad_hashes() {

...
const labels:Record<string,string>={scheduled:'مجدول',arrived:'وصل',in_progress:'قيد الخدمة',completed:'مكتمل',cancelled:'ملغي',no_show:'لم يحضر'};
...
export function ReadOnlyAppointmentsPage(){
  ...
  return <section className="page appointmentsPage print-scope" dir="rtl" aria-busy={busy}>
    ...
    <div className="pageTitle noPrint"><div><h1>المواعيد</h1><p>عرض المواعيد المصرح بها للقراءة فقط.</p></div><button type="button" onClick={()=>window.print()}><Printer aria-hidden="true"/>طباعة اليوم</button></div>
    ...
    <div className="tableCard printTableCard">{rows.length===0?<div className="empty"><CalendarDays aria-hidden="true"/><h3>لا توجد مواعيد في هذا اليوم</h3></div>:<table><thead><tr><th>الوقت</th><th>رقم الملف</th><th>المريض</th><th>العيادة</th><th>الطبيب</th><th>الحالة</th></tr></thead><tbody>{rows.map((a)=><tr key={a.id}><td dir="ltr">{new Date(a.startsAt).toLocaleTimeString('ar-SA-u-ca-gregory',{hour:'numeric',minute:'2-digit'})}</td><td><bdi>{a.fileNo}</bdi></td><td>{a.patientName}</td><td>{a.clinicName||'—'}</td><td>{a.doctorName||'—'}</td><td><span className={`status status-${a.status}`}>{labels[a.status]||a.status}</span></td></tr>)}</tbody></table>}</div>
  </section>;
}

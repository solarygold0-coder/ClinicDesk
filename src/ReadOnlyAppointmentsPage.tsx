...
export function ReadOnlyAppointmentsPage(){
  ...
  return <section className="page appointmentsPage print-scope" dir="rtl" aria-busy={busy}>
    ...
    <div className="tableCard printTableCard">{rows.length===0?<div className="empty"><CalendarDays aria-hidden="true"/><h3>لا توجد مواعيد في هذا اليوم</h3></div>:<table><thead><tr><th>الوقت</th><th>رقم الملف</th><th>المريض</th><th>العيادة</th><th>الطبيب</th><th>الحالة</th></tr></thead><tbody>{rows.map((a)=><tr key={a.id}><td dir="ltr">{fullGregorianDateTime(a.startsAt)}</td><td><bdi>{a.fileNo}</bdi></td><td>{a.patientName}</td><td>{a.clinicName||'—'}</td><td>{a.doctorName||'—'}</td><td><span className={`status status-${a.status}`}>{labels[a.status]||a.status}</span></td></tr>)}</tbody></table>}</div>
  </section>;
}

...
const files = {
  ...
  dateTimeSpinner: read('src/DateTimeSpinner.tsx'),
...
const checks = [
  ...
  ['temporary passwords must be replaced before application access', files.auth.includes('mustChangePassword') && files.auth.includes('تغيير كلمة المرور المؤقتة') && t.auth.includes('api.changePassword(pendingSession.token,password,newPassword)') && t.api.includes("changePassword:(token:string,currentPassword:string,newPassword:string)=>invoke<UserSummary>('auth_change_password',{actorToken:token,currentPassword,newPassword})")],
  ...
  ['users permissions and lifecycle are inside the settings center', files.settingsHub.includes('المستخدمون والصلاحيات') && files.settingsHub.includes('سجل العمليات') && t.app.includes('<SettingsHubcurrentUser={session.user}canManage={canManage}/>')],
  ...
  ['provider unavailability schema remains registered through schema 18', files.providerMigration.includes('provider_unavailability_events') && files.providerMigration.includes('provider_unavailability_actions') && files.runtime.includes('const LATEST_SCHEMA_VERSION: i64 = 18;') && files.runtime.includes('../migrations/017_provider_unavailability.sql') && files.runtime.includes('../migrations/018_exact_four_password_policy.sql') && files.runtime.includes('provider_unavailability_resolve_many')],
  ...
  ['patient birth dates start at 1950', files.patients.includes('DateTimeSpinner') && files.dateTimeSpinner.includes('min={1950}')],
  ['patient birth dates end at 2050', files.patients.includes('DateTimeSpinner') && files.dateTimeSpinner.includes('max={2050}')],
  ['appointment workday dates are bounded', files.appointments.includes('DateTimeSpinner') && files.dateTimeSpinner.includes('min={1950}') && files.dateTimeSpinner.includes('max={2050}')],
  ['appointment follow-up dates are bounded', files.appointments.includes('mode="datetime"') && files.dateTimeSpinner.includes('min={1950}') && files.dateTimeSpinner.includes('max={2050}')],
  ['closure dates are bounded', files.scheduling.includes('DateTimeSpinner') && files.dateTimeSpinner.includes('min={1950}') && files.dateTimeSpinner.includes('max={2050}')],
  ...
  ['patient record uses guarded print action', t.patients.includes('onClick={()=>voidsafePrint()}') && files.patients.includes('طباعة الملف') && files.safePrint.includes('onCloseRequested')],
  ['appointment day uses guarded print action', t.appointments.includes('onClick={()=>voidsafePrint()}') && files.appointments.includes('طباعة اليوم') && files.safePrint.includes('clinicdeskPrinting')],
  ...
  ['attachments use verified backend opening and block duplicate mutation UI', files.attachments.includes('api.openAttachment(item.id)') && files.attachments.includes('برنامج في Windows') && files.attachments.includes('aria-busy={busy}') && t.attachments.includes('if(busy') && t.api.includes("openAttachment:(id:number)=>authed<void>('attachment_open',{id})")],
